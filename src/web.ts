...
import type { NostrSignerPlugin, Permission } from './definitions';
...
{
  async setPackageName(_options: { packageName: string }): Promise<void> {
  ...
  async nip04Encrypt(options: {
    plainText: string;
    pubKey: string;
  }): Promise<{ result: string }> {
    if (
      this.isNostrNip04Available() &&
      typeof window.nostr.nip04.encrypt === 'function'
    ) {
      try {
        const result = await window.nostr.nip04.encrypt(
          options.pubKey,
          options.plainText,
        );
        return { result };
  ...
  async nip04Decrypt(options: {
    ...
    if (
      ...
      try {
        const result = await window.nostr.nip04.decrypt(
          options.pubKey,
          options.encryptedText,
        );
        return { result };
      } catch (error) {
        throw this.convertError(error);
  ...
  async nip44Encrypt(options: {
    plainText: string;
    pubKey: string;
  }): Promise<{ result: string }> {
    if (
      this.isNostrNip44Available() &&
      typeof window.nostr.nip44.encrypt === 'function'
    ) {
      try {
        const result = await window.nostr.nip44.encrypt(
          options.pubKey,
          options.plainText,
        );
        return { result };
  ...
  async nip44Decrypt(options: {
    ...
    if (
      ...
      try {
        const result = await window.nostr.nip44.decrypt(
          options.pubKey,
          options.encryptedText,
        );
        return { result };
  ...
  async decryptZapEvent(_options: {
    eventJson: string;
  }): Promise<{ result: string }> {
